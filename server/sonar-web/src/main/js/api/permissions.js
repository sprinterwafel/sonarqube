...
export function getPermissionsUsersForComponent(
  ...
  const data: Object = { projectKey, permission, organization, ps: PAGE_SIZE, q: query };
...
export function getGlobalPermissionsUsers(
  ...
  const url = '/api/permissions/users';
  const data: Object = { permission, organization, ps: PAGE_SIZE, q: query };
  return getJSON(url, data).then(r => r.users);
}
...
export function getPermissionTemplateUsers(
  ...
  const data: Object = { templateId, permission, organization, ps: PAGE_SIZE, q: query };
  return getJSON(url, data).then(r => r.users);
}
